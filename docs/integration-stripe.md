...
export default async function handler(req: Request): Promise<Response> {
  ...
  const result = await verifiers.verify(rawBody, { ...headers, 'x-provider': 'stripe' });

  if (result.status === 'success') {
    // Fulfill the order. The result is the normalized PaymentResult shape.
    await markPaid(result.paymentId, result.amount);
  }
