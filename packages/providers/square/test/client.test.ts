...
import { createSquareProvider } from '../src/index.js';

describe('createSquareProvider', () => {
	it('POSTs to the configured session endpoint and navigates to the returned url', async () => {
		...
		const provider = createSquareProvider({
			sessionEndpoint: 'http://test/api/checkout/square',
			fetcher: fetchStub,
		...
		expect(calledUrl).toBe('http://test/api/checkout/square');
		...
		expect(navigateStub).toHaveBeenCalledWith('https://square.link/u/EXAMPLE');
	...
	it('uses the real window.location.assign when no navigate override is provided', async () => {
		...
		try {
			const fetchStub = vi.fn(
				async () =>
					new Response(JSON.stringify({ url: 'https://square.link/u/X' }), { status: 200 }),
			);
			const provider = createSquareProvider({
				sessionEndpoint: 'http://test/checkout',
				fetcher: fetchStub,
			...
			expect(assignSpy).toHaveBeenCalledWith('https://square.link/u/X');
		} finally {
	...
	it('succeeds without calling onPending when the callback is not provided', async () => {
		const fetchStub = vi.fn(
			async () => new Response(JSON.stringify({ url: 'https://square.link/u/Y' }), { status: 200 }),
		);
		const navigateStub = vi.fn();
		const provider = createSquareProvider({
			sessionEndpoint: 'http://test/checkout',
		...
		expect(navigateStub).toHaveBeenCalledWith('https://square.link/u/Y');
	});
