...
describe('createStripeProvider (browser)', () => {
	...
	it('succeeds without calling onPending when the callback is not provided', async () => {
		vi.spyOn(globalThis, 'fetch').mockResolvedValue(
			new Response(JSON.stringify({ url: 'https://checkout.stripe.com/c/pay/cs_abc' }), {
				status: 200,
			}),
		);
		const provider = createStripeProvider();
		...
		expect(assignMock).toHaveBeenCalledWith('https://checkout.stripe.com/c/pay/cs_abc');
	});

	it('rejects redirect URLs with non-http(s) schemes', async () => {
		vi.spyOn(globalThis, 'fetch').mockResolvedValue(
			new Response(JSON.stringify({ url: 'javascript:alert(1)' }), { status: 200 }),
		);
		const provider = createStripeProvider();
		const onError = vi.fn();
		...
		expect(onError).toHaveBeenCalledOnce();
		expect((onError.mock.calls[0][0] as Error).message).toMatch(/not allowed/);
		expect(assignMock).not.toHaveBeenCalled();
	});
