 ...
 * own backend, receives an `invoice_url`, and redirects the buyer there.
 * NOWPayments handles the coin selection, blockchain monitoring, and
 * forwarding to the merchant's payout wallet.
 *
 ...
 *       checkoutEndpoint: '/api/checkout/nowpayments',
 *     });
 *
...
function assertSafeRedirectUrl(url: string): void {
	...
	if (parsed.protocol !== 'https:' && parsed.protocol !== 'http:') {
		throw new Error(
			`coin-moebius/nowpayments: redirect URL scheme "${parsed.protocol}" is not allowed`,
		);
	}
}
