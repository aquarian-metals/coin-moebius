...
export function markReceived(
	...
	// Manual transactions don't flow through the verifier registry — there's
	// no webhook to dispatch. The caller fires this `PaymentResult` on their
	// own status endpoint directly. Subscription events are not applicable
	// to the manual rail, so we stay on the focused `PaymentResult` shape.
