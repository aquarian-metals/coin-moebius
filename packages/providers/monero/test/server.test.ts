...
describe('createMoneroVerifier', () => {
	...
	it('accepts a correctly-signed success payload and maps it to PaymentResult', async () => {
		...
		const result = await verifier.verify(payload, { 'x-monero-sig': sig });
	...
	it('accepts the body as a raw JSON string identically to a parsed object', async () => {
		...
		const fromObject = await verifier.verify(payload, { 'x-monero-sig': sig });
		const fromString = await verifier.verify(bodyString, { 'x-monero-sig': sig });
