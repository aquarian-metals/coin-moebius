...
import { createCoinbaseBusinessProvider } from '../src/index.js';
 ...
 * so we can assert it was called with the hosted_url returned by the
 * session endpoint.
 */

describe('createCoinbaseBusinessProvider', () => {
	it('POSTs to the configured session endpoint and navigates to the returned url', async () => {
		const fetchStub = vi.fn(
			async (_url: RequestInfo | URL, _init?: RequestInit) =>
				new Response(
					JSON.stringify({
						url: 'https://business.coinbase.com/checkout/abc123',
						paymentId: 'cb_checkout_abc123',
					}),
		...
		const provider = createCoinbaseBusinessProvider({
			sessionEndpoint: 'http://test/api/checkout/coinbase-business/proj_x',
			fetcher: fetchStub,
		...
		expect(calledUrl).toBe('http://test/api/checkout/coinbase-business/proj_x');
		...
		expect(navigateStub).toHaveBeenCalledWith('https://business.coinbase.com/checkout/abc123');
	...
	it('uses the real window.location.assign when no navigate override is provided', async () => {
		...
		try {
			const fetchStub = vi.fn(
				async () =>
					new Response(JSON.stringify({ url: 'https://business.coinbase.com/cb/x' }), {
						status: 200,
					}),
			);
			const provider = createCoinbaseBusinessProvider({
			...
			expect(assignSpy).toHaveBeenCalledWith('https://business.coinbase.com/cb/x');
		} finally {
	...
	it('succeeds without calling onPending when the callback is not provided', async () => {
		...
		expect(navigateStub).toHaveBeenCalledWith('https://business.coinbase.com/cb/x');
	});
