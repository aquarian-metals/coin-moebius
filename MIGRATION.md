...
**If you read `result.status` directly:**
...
**Or use the narrowing helpers** (cleaner for code that only cares about one shape):
...
if (payment?.status === 'success') {
  // payment is PaymentResult-shaped here, no `kind` field
}

const sub = asSubscription(event);
if (sub?.type === 'subscription.renewed') {
  // sub is SubscriptionEvent-shaped here, no `kind` field
}
```

**Five new subscription event types** are emitted by providers that support recurring billing (Stripe today, PayPal/Square/Authorize.net to follow):

- `subscription.created` — initial signup
- `subscription.renewed` — non-initial billing cycle paid
- `subscription.payment_failed` — billing cycle failed (the provider's dunning will retry on its schedule)
- `subscription.canceled` — terminal cancellation
- `subscription.updated` — status / plan / card change

Subscription events carry `subscriptionId`, `productId`, `customerRef`, normalized `status`, and `currentPeriodEnd`. See `SubscriptionEvent` in `@aquarian-metals/coin-moebius-core`.

**No changes required** if you don't use subscriptions. The verifier emits payment events exactly as before, just wrapped with `kind: 'payment'`.
